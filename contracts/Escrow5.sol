...
contract Escrow5 {
    ...
    function confirmDelivery(address NFT, uint256 index) external payable { //contract releases NFT to buyer, and contract releases payment to seller
        ...
        uint256 amountToSeller = Ecrows[NFT][index].price * 1e18 * pandoPercentage / 10000; 
        uint256 amountToPando = (Ecrows[NFT][index].price *1e18) - amountToSeller; 
    ...
    function declinePendingOffer(address NFT, uint256 index) external{
        ...
        payable(buyer).transfer(Ecrows[NFT][index].price * 1e18);
